// Patches alle 5 digipub_leistungen-Einträge mit korrekt kodierten Daten
...
const items = [
  ...
  },
  {
    ...
    section1_text: "GEO ist ohne SEO nicht zu denken. Bevor Inhalte in generativen Antworten erscheinen können, müssen die grundlegenden SEO-Regeln erfüllt sein: technische Zugänglichkeit, klare Struktur, inhaltliche Relevanz und eine saubere Nutzerführung.",
    section1_bold: "GEO erweitert die bekannten SEO-Best Practices um die Frage, wie Inhalte zitiert, zusammengefasst und eingeordnet werden.",
    section2_title: "SEO & GEO als Rückkopplung",
    section2_text: "GEO verändert nicht nur Sichtbarkeit, sondern Arbeitsweisen. Inhalte entstehen näher an der Praxis, Answer-first und zunehmend aus erster Hand. Diese Rückkopplung wirkt ins Unternehmen zurück: Praxiswissen wird sichtbar, Qualität und Aktualität werden verbindlich.",
    section2_bold: "Sichtbarkeit wird zur Vertrauensfrage, nicht zur Rankingfrage.",
    ansatz_title: "Unser Ansatz",
    ...
    leistungen_cards: [
      ...
      { title: "Technisches Fundament: SEO-Basics für GEO", text: "Struktur & Informationsarchitektur (Themencluster, interne Verlinkung), strukturierte Daten / Schema (z.B. FAQ/HowTo/Produkt), Performance & Mobile UX als Grundlage sowie KI-Traffic-Fallen vermeiden.", ziel: "Saubere Basis, auf der GEO-Maßnahmen greifen." },
      ...
      { title: "Plattform- & Offpage-Strategie", text: "Digital PR & Platzierungen (Fachmedien, Rankings, Best of, Interviews), Community-/Foren-Strategie (hilfreiche Beiträge, echte Expertise), Reputation: Reviews/Testimonials systematisch fördern sowie Knowledge-/Entity-Pflege.", ziel: "Erwähnungen, die KI aufgreift und weitergibt." },
      ...
      { title: "Tracking & Analyse: KI-Sichtbarkeit messbar machen", text: "KPIs wie Mention/Citation-Tracking & Share of Voice in generativen Antworten, Tool-Setups/Workflows (z.B. SISTRIX AI Overviews, Ahrefs Brand Radar) sowie Monitoring der Volatilität & schnelle Nachsteuerung.", ziel: "Transparenz über SEO- und GEO-Performance in einem System." },
    ],
    wieso_title: "Wieso mit DigiPub?",
  ...
  },
  {
    ...
    section2_text: "Ein Designsystem ist mehr als eine Sammlung schöner Komponenten. Gestaltung ergibt nur dann Sinn, wenn das Konzept hinter dem Inhalt klar ist. Ein Konzept muss verständlich sein, Informationen brauchen Hierarchie, Rhythmus und Gewicht.",
    ...
    leistungen_cards: [
      { title: "Designsystem-Strategie & Systemarchitektur", text: "Wir definieren die Struktur, auf der alle Marken, Anwendungen und Touchpoints aufbauen: Designsystem-Ziele & Einsatzkontexte, Multibrand-Logik (Core-System + markenspezifische Layer), Regeln für Konsistenz und bewusste Abweichung, Hierarchien, Modularität & Skalierbarkeit sowie Governance.", ziel: "Ein belastbares System, das Orientierung schafft, Wildwuchs verhindert und mit dem Unternehmen mitwächst." },
      ...
      { title: "Design-Tokens, Komponenten- & Layoutsysteme", text: "Wir übersetzen Konzept und visuelle Regeln in konkrete, wiederverwendbare Bausteine: Design-Tokens (Farben, Typografie, Spacing etc.), markenspezifische Variablen, UI-Komponenten, modulare Layoutsysteme sowie Vorlagen für wiederkehrende Formate.", ziel: "Schneller arbeiten, weniger Reibung, gleichbleibende Qualität, unabhängig von Marke oder Kanal." },
      { title: "Figma-Implementierung, Kollaboration & Enablement", text: "Figma als Single Source of Truth, zentrale Libraries & Multibrand-Setups mit Variablen, gemeinsame Nutzung für Design, Marketing & Content, Dokumentation direkt im System sowie Einführung, Schulung & Befähigung der Teams.", ziel: "Ein lebendiges Designsystem, das genutzt, gepflegt und eigenständig weiterentwickelt wird." },
  ...
  },
  {
    ...
    section1_text: "Automatisierung verspricht Entlastung, erzeugt aber oft Unsicherheit: Was automatisieren wir? Was nicht? Wo lohnt sich der Aufwand wirklich? Viele Unternehmen spüren: Es fehlt Zeit, Prozesse sind langsam oder fehleranfällig, Mitarbeitende sind mit operativen Aufgaben gebunden.",
  ...
  },
  {
    ...
    section2_text: "KI-Projekte scheitern oft am Big-Bang-Versuch. Wir setzen auf einen modularen Aufbau, der Vertrauen schafft: 1. Governance & Compliance, 2. Guided Prompting, 3. Spezialisierte Assistenten, 4. Skalierung & Agenten (erst wenn die Basis sicher steht).",
...
for (const item of items) {
  const { id, ...data } = item;
  const res = await fetch(`${BASE}/items/digipub_leistungen/${id}`, {
    method: "PATCH",
    headers: {
  ...
  if (json.errors) {
    console.error(`ID ${id}:`, json.errors[0].message);
  } else {
    console.log(`✓ ID ${id} (${item.slug}) aktualisiert`);
  }
}
